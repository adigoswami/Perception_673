...
# set the path to the folder for the training dataset (green dataset for training)
path = glob.glob(r"C:\Users\sukoo\673\Project3\Training_Data\Green_Training\*.jpg")
...
while(c!=50): #iterations to be run 50 times
    ...
    for img in path: 
        ...
        img = cv2.imread(img)
        # taking  green channel into account for green buoy
        b_img = img[:,:,0].astype(np.float32)
        g_img = img[:,:,1].astype(np.float32)
        flat_array = list(g_img.flatten()) #flatten and convert into list
        ...
        for i in flat_array: #over every pixel value in flat_array
            ...
            prob_2 = prob_gaussian(mu2, var2, i) #find probability 2 for gaussian 1 using function : prob_gaussian(mean, variance, value in flat_array)
            prob_3 = prob_gaussian(mu3, var3, i) #find probability 3 for gaussian 1 using function : prob_gaussian(mean, variance, value in flat_array)
...
#Plotting the Gaussian for green bouy
plt.figure(figsize=(10,7))
plt.xlabel("$x$")
plt.ylabel("pdf")
plt.scatter(X, [0.005] * len(X), color='navy', s=30, marker=2, label="Train data")

plt.plot(bins, pdf(bins, mu1, var1), color='green', label="True pdf")
plt.plot(bins, pdf(bins, mu2, var2), color='green')
plt.plot(bins, pdf(bins, mu3, var3), color='green')

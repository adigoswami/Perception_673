...
# set the path to the folder for the training dataset (yellow dataset for training)
...
while(c!=50): #iterations to be run 50 times
    ...
    for img in path: 
        ...
        img = cv2.imread(img)
        # taking red and green channel out for yellow (similarly take green and blue channels into account for green buoy, and red channel for orange buoy)
        r_img = img[:,:,2].astype(np.float32)
        g_img = img[:,:,1].astype(np.float32)
        addition = (r_img + g_img)/2   #gives out mean/average channel array
        flat_array = list(addition.flatten()) #flatten and convert into list
        ...
        for i in flat_array: #over every pixel value in flat_array
            ...
            prob_2 = prob_gaussian(mu2, var2, i) #find probability 2 for gaussian 2 using function : prob_gaussian(mean, variance, value in flat_array)
            prob_3 = prob_gaussian(mu3, var3, i) #find probability 3 for gaussian 3 using function : prob_gaussian(mean, variance, value in flat_array)
...
#Plotting the Gaussian for yellow bouy
plt.figure(figsize=(10,7))
plt.xlabel("$x$")
plt.ylabel("pdf")
plt.scatter(X, [0.005] * len(X), color='navy', s=30, marker=2, label="Train data")

plt.plot(bins, pdf(bins, mu1, var1), color='yellow', label="True pdf")
plt.plot(bins, pdf(bins, mu2, var2), color='yellow')
plt.plot(bins, pdf(bins, mu3, var3), color='yellow')
